...
struct DisplayThread : Thread
{
  ...
  void step()
  {
    ...
    if (numSolutions)
      gl.update();
    if (saveVideo)
      write_ppm(gl.captureImage, STRING(OptLGPDataPath << "vid/" << std::setw(4) << std::setfill('0') << t++ << ".ppm"));
  }
};
...
void initFolStateFromKin(FOL_World &L, const rai::Configuration &K)
{
  ...
  for (rai::Frame *a : K.frames)
    if (a->shape && a->ats && (*a->ats)["logical"])
    {
      ...
      for (rai::Frame *b : F)
        if (b != a && b->shape && a->ats && (*b->ats)["logical"])
        {
  ...
  for (rai::Frame *a : K.frames)
    if (a->shape && a->ats && (*a->ats)["logical"])
    {
      ...
      for (rai::Frame *b : F)
        if (b != a && b->shape && b->ats && (*b->ats)["logical"])
        {
          L.addFact({"on", b->name, a->name});
        }
    }
  // adding equalities to init state
  for (rai::Frame *a : K.frames)
    if (a->shape && a->ats && (*a->ats)["logical"])
    {
      L.addFact({"equal", a->name, a->name});
    }
}
    ...
    : verbose(2), numSteps(0)
{
  ...
  if (!filNodes)
    filNodes = new ofstream(dataPath + "nodes");
  ...
  if (verbose > 0)
    fil.open(dataPath + "optLGP.dat"); // STRING("z.optLGP." <<rai::date() <<".dat"));
...
LGP_Tree::LGP_Tree(const rai::Configuration &_kin, const char *folFileName) : LGP_Tree()
{
  kin.copy(_kin);
  if (collisions)
    kin.swift(); // initialize swift in root model (SwiftInterface is reference by all child models)
...
void LGP_Tree::displayTreeUsingDot()
{
  ...
  for (auto &n : all)
    if (n->isInfeasible)
      n->note << "INFEASIBLE ";
  for (auto &n : fringe_expand)
    n->note << "EXPAND ";
  for (auto &n : terminals)
    n->note << "TERMINAL ";
  for (auto &n : fringe_pose)
    n->note << "POSE ";
  for (auto &n : fringe_poseToGoal)
    n->note << "POSE2 ";
  for (auto &n : fringe_seq)
    n->note << "SEQ ";
  for (auto &n : fringe_path)
    n->note << "PATH ";
  for (auto &n : fringe_solved)
    n->note << "DONE";
...
void LGP_Tree::updateDisplay()
{
    ...
    for (uint i = 1; i < views.N; i++)
    {
      if (focusNode->komoProblem(i) && focusNode->komoProblem(i)->timeSlices.N)
      {
        NIY // views(i)->setConfigurations(focusNode->komoProblem(i)->configurations);
                views(i)
                    ->text.clear()
            << focusNode->cost << "|  " << focusNode->constraints.last() << '\n'
            << decisions;
      }
      else
        views(i)->clear();
  ...
  for (uint i = 0; i < solutions().N && i < 6; i++)
  {
    if (dth->gl.views.N <= i || !dth->gl.views(i).drawers.N)
    {
      ...
      if (cameraFocus.N)
        dth->gl.views(i).camera.focus(cameraFocus, true);
...
LGP_Node *LGP_Tree::walkToNode(const rai::String &seq)
{
  ...
  for (Node *actionLiteral : tmp)
  {
    ...
    if (!next)
      LOG(-2) << "action '" << *actionLiteral << "' is not a child of '" << *node << "'";
    node = next;
  }
...
void LGP_Tree::optFixedSequence(const rai::String &seq, BoundType specificBound, bool collisions)
{
  ...
  if (specificBound == BD_all || specificBound == BD_pose)
    node->optBound(BD_pose, collisions, verbose - 2);
  if (specificBound == BD_all || specificBound == BD_seq)
    node->optBound(BD_seq, collisions, verbose - 2);
  if (specificBound == BD_all || specificBound == BD_path)
    node->optBound(BD_path, collisions, verbose - 2);
  if (specificBound == BD_all || specificBound == BD_seqPath)
    node->optBound(BD_seqPath, collisions, verbose - 2);
...
{
  for (const rai::String &seq : seqs)
    optFixedSequence(seq);
...
bool LGP_Tree::execChoice(rai::String cmd)
{
  ...
  else if (cmd == "u")
  {
    ...
    focusNode->optBound(BD_pose, collisions, verbose - 2);
  else if (cmd == "s")
    focusNode->optBound(BD_seq, collisions, verbose - 2);
  else if (cmd == "x")
    focusNode->optBound(BD_path, collisions, verbose - 2);
...
LGP_Node *LGP_Tree::getBest(LGP_NodeL &fringe, int level)
{
  ...
  {
    if (n->isInfeasible || !n->count(level))
      continue;
...
LGP_Node *LGP_Tree::expandNext(int stopOnDepth, LGP_NodeL *addIfTerminal)
{ // expand
  if (!fringe_expand.N)
    HALT("the tree is dead!");
  ...
  CHECK(n, "");
  if (n->children.N)
    return nullptr; // if we already saw this node then dont expand it again
  if (stopOnDepth > 0 && n->step >= (uint)stopOnDepth)
    return nullptr;
  n->expand();
  for (LGP_Node *ch : n->children)
  {
    if (heuristicCosts)
      heuristicCosts(ch); // here we compute the heuristic costs and write them into ch->costs(BD_sym)
    if (ch->isTerminal)
    {
      terminals.append(ch);
    ...
    if (addIfTerminal && ch->isTerminal)
      addIfTerminal->append(ch);
  }
  return n;
}

void LGP_Tree::optBestOnLevel(BoundType bound, LGP_NodeL &drawFringe, BoundType drawFrom, LGP_NodeL *addIfTerminal, LGP_NodeL *addChildren)
{ // optimize a seq
  if (!drawFringe.N)
    return;
  ...
  if (n && !n->count(bound))
  {
    ...
    if (n->feasible(bound))
    {
      ...
      if (addChildren)
        for (LGP_Node *c : n->children)
          addChildren->append(c);
    }
...
rai::String LGP_Tree::report(bool detailed)
{
  ...
  if (detailed)
  {
    ...
    for (LGP_Node *n : fringe_solved)
      n->write(out, false, true);
  }
...
void LGP_Tree::step()
{
  ...
  if (fringe_solved.N > numSol)
  {
    if (verbose > 0)
    {
      cout << "NEW SOLUTION FOUND! " << fringe_solved.last()->getTreePathString();
      cout << "LEN: " << fringe_solved.last()->step;
      cout << endl;
    }
    ...
    {
      BoundType bound;
      if (focusNode->komoProblem(BD_seqPath))
        bound = BD_seqPath;
      else if (focusNode->komoProblem(BD_path))
        bound = BD_path;
      else
        HALT("NO KOMO FOUND");
...
void LGP_Tree::getSymbolicSolutions(uint depth)
{
  ...
  if (!terminals.N)
    cout << "NO SOLUTIONS up to depth " << depth << endl;
}
...
LGP_Tree_SolutionData::LGP_Tree_SolutionData(LGP_Tree &_tree, LGP_Node *_node) : tree(_tree), node(_node)
{
  ...
  for (uint i = 0; i < geoms.N; i++)
    geoms(i) = K.frames(frameIDs(i))->shape->_mesh;
...
void LGP_Tree_SolutionData::glDraw(OpenGL &gl)
{
  ...
  if (!paths(displayBound).N)
    return;

  for (uint i = 0; i < geoms.N; i++)
  {
    if (displayStep >= paths(displayBound).d0)
      displayStep = 0;
